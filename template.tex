...
\Title{Information Evolution by Stability-Driven Assembly}


% MDPI internal command: Title for citation in the left column
\TitleCitation{Information Evolution by Stability-Driven Assembly}
...
\abstract{
Information can evolve even in the absence of genes or replication. We present Stability-Driven Assembly (SDA), a framework in which persistence imbalances bias populations toward longer-lived motifs, generating selection-like dynamics. In SDA, assemblies are created stochastically and persist according to a stability function. Assemblies that endure longer are more likely to participate in future interactions. This feedback loop yields fitness-proportional sampling, so that a natural genetic algorithm (SDA/GA) emerges, driven solely by stability. We apply the model in chemical symbol space using SMILES fragments, recombination, mutation, and a heuristic stability function. Simulations display the hallmarks of selection: populations become dominated by a few stable compounds sharing common scaffolds, with a corresponding reduction in Shannon entropy. In contrast, chemical equilibrium models with fixed rates lack such feedback, relaxing to uniform steady states rather than modeling open-ended evolution. SDA/GA thus provides a general model of information evolution: whenever stochastic assembly, replenishment, and differential persistence coexist, populations become skewed and ordered structures accumulate.
}
